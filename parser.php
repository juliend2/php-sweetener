...
class Parser {
  ...
  private function add_semicolons($input_lines) {
    ...
    foreach ($input_lines as $key => $line) {
      if (
         trim($line) == '' # is empty
      || preg_match('/^(class|function|if|else|elseif|while|for|foreach|public|private|protected|static|interface|abstract)\s?/', trim($line)) # starts with a keyword that precedes something that doesnt end with a semicolon
